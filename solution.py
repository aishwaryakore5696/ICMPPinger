...
def ping(host, timeout=1):
    ...
    variance = sum([((x - mean) ** 2) for x in delaylist]) / len(delaylist) 
    sd = variance ** 0.5
    vars = [str(round(min(delaylist)*1000, 2)), str(round(mean*1000, 2)), str(round(max(delaylist)*1000, 2)),str(round(sd*1000, 2))]
    #print("vars",vars)
    return vars
